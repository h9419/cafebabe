...
object AbstractByteCodes {
  ...
  trait AbstractByteCode extends Streamable {
    def size: Int
    def toStream(bs: ByteStream): ByteStream =
      sys.error("Abstract byte code cannot produce its own code. " + this)
  }

  /** Line numbers take no space in the actual bytecode. They are stored (at
    * freeze-time) in the line number table and useful for debugging and for
  ...
  /** A label takes no space in the actual bytecode, but serves as an anchor in
    * the stream of ABC.
    */
  ...
  abstract class ControlOperator(val opCode: ByteCode)
      extends AbstractByteCode {
    def size: Int = 3
    val target: String
    var offset: Int = _
  ...
  case class IfNonNull(override val target: String)
      extends ControlOperator(IFNONNULL)
  case class If_ICmpEq(override val target: String)
      extends ControlOperator(IF_ICMPEQ)
  case class If_ICmpNe(override val target: String)
      extends ControlOperator(IF_ICMPNE)
  case class If_ICmpLt(override val target: String)
      extends ControlOperator(IF_ICMPLT)
  case class If_ICmpLe(override val target: String)
      extends ControlOperator(IF_ICMPLE)
  case class If_ICmpGt(override val target: String)
      extends ControlOperator(IF_ICMPGT)
  case class If_ICmpGe(override val target: String)
      extends ControlOperator(IF_ICMPGE)
  case class If_ACmpEq(override val target: String)
      extends ControlOperator(IF_ACMPEQ)
  case class If_ACmpNe(override val target: String)
      extends ControlOperator(IF_ACMPNE)

  type AbstractByteCodeGenerator = (CodeHandler => CodeHandler)

  /** Generates code to load constants, using the appropriate method depending
    * on the values.
    */
  object Ldc {
    def apply(i: Int): AbstractByteCodeGenerator = ((ch: CodeHandler) => {
      i match {
        ...
        case 4  => ch << ICONST_4
        case 5  => ch << ICONST_5
        case _ if (i >= -128 && i <= 127) =>
          ch << BIPUSH << RawByte(i.asInstanceOf[U1])
        case _ if (i >= -32768 && i <= 32767) =>
          ch << SIPUSH << RawBytes(i.asInstanceOf[U2])
    ...
    def apply(s: String): AbstractByteCodeGenerator = ((ch: CodeHandler) => {
      ch << ldc_ref(
        ch.constantPool.addStringConstant(ch.constantPool.addString(s))
      )
    })

    def apply(c: Class[_]): AbstractByteCodeGenerator = ((ch: CodeHandler) => {
      ch << ldc_ref(
        ch.constantPool.addClass(
          ch.constantPool.addString(c.getName().replaceAll("\\.", "/"))
        )
      )
    })

    private def ldc_ref(cpRef: U2): AbstractByteCodeGenerator =
      ((ch: CodeHandler) => {
        if (cpRef <= 0xff) {
    ...
    private def ldc2_ref(cpRef: U2): AbstractByteCodeGenerator =
      ((ch: CodeHandler) => {
  ...
  // IINC business
  object IInc {
    def apply(index: Int, inc: Int): AbstractByteCodeGenerator =
      ((ch: CodeHandler) => {
        if (index <= 127 && inc >= -128 && inc <= 127) {
          ch << IINC << RawByte(index.asInstanceOf[U1]) << RawByte(
            inc.asInstanceOf[U1]
          )
        } else if (index <= 32767 && inc >= -32768 && inc <= 32767) {
          ch << WIDE << IINC << RawBytes(index.asInstanceOf[U2]) << RawBytes(
            index.asInstanceOf[U2]
          )
        } else {
  ...
  private def storeLoad(
    ...
    index match {
      ...
      case 2 => ch << bc2
      case 3 => ch << bc3
      case _ if (index >= 0 && index <= 127) =>
        ch << bc << RawByte(index.asInstanceOf[U1])
      case _ if (index >= 0 && index <= 32767) =>
  ...
  object ALoad {
    def apply(index: Int) =
      storeLoad(index, "ALoad", ALOAD, ALOAD_0, ALOAD_1, ALOAD_2, ALOAD_3)
  }
  object DLoad {
    def apply(index: Int) =
      storeLoad(index, "DLoad", DLOAD, DLOAD_0, DLOAD_1, DLOAD_2, DLOAD_3)
  }
  object FLoad {
    def apply(index: Int) =
      storeLoad(index, "FLoad", FLOAD, FLOAD_0, FLOAD_1, FLOAD_2, FLOAD_3)
  }
  object ILoad {
    def apply(index: Int) =
      storeLoad(index, "ILoad", ILOAD, ILOAD_0, ILOAD_1, ILOAD_2, ILOAD_3)
  }
  object LLoad {
    def apply(index: Int) =
      storeLoad(index, "LLoad", LLOAD, LLOAD_0, LLOAD_1, LLOAD_2, LLOAD_3)
  }
  object AStore {
    def apply(index: Int) =
      storeLoad(index, "AStore", ASTORE, ASTORE_0, ASTORE_1, ASTORE_2, ASTORE_3)
  }
  object DStore {
    def apply(index: Int) =
      storeLoad(index, "DStore", DSTORE, DSTORE_0, DSTORE_1, DSTORE_2, DSTORE_3)
  }
  object FStore {
    def apply(index: Int) =
      storeLoad(index, "FStore", FSTORE, FSTORE_0, FSTORE_1, FSTORE_2, FSTORE_3)
  }
  object IStore {
    def apply(index: Int) =
      storeLoad(index, "IStore", ISTORE, ISTORE_0, ISTORE_1, ISTORE_2, ISTORE_3)
  }
  object LStore {
    def apply(index: Int) =
      storeLoad(index, "LStore", LSTORE, LSTORE_0, LSTORE_1, LSTORE_2, LSTORE_3)
  }
...
// Some magic for loading locals.
  ...
  object ArgLoad {
    ...
    /** Loads an argument by its index in the argument list. 0 is the receiver
      ...
      */
    def apply(index: Int): AbstractByteCodeGenerator = (ch: CodeHandler) =>
  ...
  private def invokeMethod(
    ...
    {
      val addMethodRef =
        if (bc == INVOKEINTERFACE) ch.constantPool.addInterfaceMethodRef _
        else ch.constantPool.addMethodRef _
      ch << bc << RawBytes(
        addMethodRef(
          ch.constantPool.addClass(ch.constantPool.addString(className)),
          ch.constantPool.addNameAndType(
      ...
      (ch: CodeHandler) =>
        {
          ch << NEW << RawBytes(
            ch.constantPool.addClass(ch.constantPool.addString(className))
          ) << DUP << InvokeSpecial(className, "<init>", "()V")
        }
    }
  }
  ...
  object NewArray {
    def apply(arrayType: String): AbstractByteCodeGenerator = { // For objects
      (ch: CodeHandler) =>
        {
          ch << ANEWARRAY << RawBytes(
            ch.constantPool.addClass(ch.constantPool.addString(arrayType))
          )
        }
    }
