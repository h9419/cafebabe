...
/** A code handler contains methods to help the generation of method bodies. The
  * general usage is to generate abstract byte codes, then freeze the code,
  ...
  * <code>CodeHandler</code>s should not be created manually, but rather
  * obtained from the corresponding <code>MethodHandler</code>.
  */
class CodeHandler private[cafebabe] (
  ...
  def getFreshVar(n: Int): Int = {
    if (!(n == 1 || n == 2)) {
      throw new IllegalArgumentException(
        "Slot for variables can only be of 1 or 2 bytes."
      )
    }
    val ret = locals
    locals += n
  ...
  def getFreshLabel(prefix: String): String = {
    val postfix: Int = labelCounts.getOrElse(
      prefix, {
  ...
  def freeze: Unit = if (frozen) {
    ...
    if (!lineInfo.isEmpty) {
      val lnta = new LineNumberTableAttributeInfo(
        constantPool.addString("LineNumberTable")
      )
  ...
  def computeMaxStack(abcList: List[AbstractByteCode]): U2 = {
    ...
    def setHeight(from: Int, there: Int): Unit = {
      if (from < 0 || from >= actualSize)
        throw CodeFreezingException(
          "No bytecode at pc=" + from + ". Missing instructions?",
          abcList
        )

      if (there < 0)
        throw CodeFreezingException(
          "Negative stack height at pc=" + from + " (which is " + codeArray(
            from
          ) + ").",
          abcList
        )
      ...
      if (heightArray(from) != UninitializedHeight) { // If another paths led to the same pc.
        if (heightArray(from) == there) {
          ...
          throw CodeFreezingException(
            "Inconsistent stack height at pc=" + from + "(" + heightArray(
              from
            ) + " and " + there + ")",
            abcList
          )
      ...
      codeArray(pc) match {
        ...
        case RETURN =>
          if (there != 0)
            throw CodeFreezingException(
              "Non-empty stack after return in void method"
            )
        case ATHROW => {
        ...
        case ARETURN | DRETURN | FRETURN | IRETURN | LRETURN => {
          if (there + codeArray(pc).asInstanceOf[ByteCode].stackEffect.get != 0)
            throw CodeFreezingException(
              "Stack not empty after return.",
              abcList
            )
        }
        case bc: ByteCode if !bc.stackEffect.isEmpty =>
          setHeight(from + bc.length.get, there + bc.stackEffect.get)
        case GETFIELD =>
          codeArray(pc + 1) match {
            case RawBytes(idx) =>
              setHeight(from + 3, (there + constantPool.getFieldSize(idx)) - 1)
            case _ =>
              throw CodeFreezingException("Expected RawBytes after GETFIELD.")
          }
        case GETSTATIC =>
          codeArray(pc + 1) match {
            case RawBytes(idx) =>
            ...
            case _ =>
              throw CodeFreezingException("Expected RawBytes after GETSTATIC.")
          }
        case PUTFIELD =>
          codeArray(pc + 1) match {
            case RawBytes(idx) =>
              setHeight(from + 3, there - constantPool.getFieldSize(idx) - 1)
            case _ =>
              throw CodeFreezingException("Expected RawBytes after PUTFIELD.")
          }
        case PUTSTATIC =>
          codeArray(pc + 1) match {
            case RawBytes(
              ...
              setHeight(from + 3, there - constantPool.getFieldSize(idx))
            case _ =>
              throw CodeFreezingException("Expected RawBytes after PUTSTATIC.")
          }
        case INVOKEVIRTUAL | INVOKESPECIAL =>
          codeArray(pc + 1) match {
            ...
            case _ =>
              throw CodeFreezingException(
                "Expected RawBytes after INVOKEVIRTUAL/INVOKESPECIAL."
              )
          }
        case INVOKEINTERFACE =>
          codeArray(pc + 1) match {
            case RawBytes(idx) =>
              codeArray(pc + 3) match {
                case RawByte(n) => {
                  ...
                  codeArray(pc + 4) match {
                    ...
                    case _ =>
                      throw CodeFreezingException(
                        "Expected RawByte(0) as the last param for INVOKEINTERFACE"
                      )
                  }
                }
                case b =>
                  throw CodeFreezingException(
                    "Expected RawByte after the RawBytes in INVOKEINTERFACE @ " + pc + " ; found: " + b
                  )

              }
            case _ =>
              throw CodeFreezingException(
                "Expected RawBytes after INVOKEINTERFACE."
              )
          }
        case INVOKESTATIC =>
          codeArray(pc + 1) match {
            ...
            case _ =>
              throw CodeFreezingException(
                "Expected RawBytes after INVOKESTATIC."
              )
          }
        ...
        case co: ControlOperator => {
          ...
          setHeight(
            from + co.opCode.length.get,
            there + co.opCode.stackEffect.get
          )
        }
        case other @ _ =>
          sys.error("Computation of stack height unsupported for " + other)
      }
    }
