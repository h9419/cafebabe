...
class ConstantPool extends Streamable {
  ...
  /** The following maps keep track of the constants already added to the pool
    * to avoid duplicates.
    */
  ...
  private val stringMap: HashMap[String, U2] =
    new HashMap[String, U2] // all internal strings
  private val stringConstMap: HashMap[U2, U2] =
    new HashMap[U2, U2] // string constants
  ...
  /** The following methods add constants to the pool, using hashmaps to avoid
    * duplicates and properly encoding the values.
    */
  def addInt(i: Int): U2 = intMap.getOrElse(
    i, {
  ...
  def addFloat(f: Float): U2 = floatMap.getOrElse(
    f, {
  ...
  def addLong(l: Long): U2 = longMap.getOrElse(
    l, {
  ...
  def addDouble(d: Double): U2 = doubleMap.getOrElse(
    d, {
  ...
  def addString(s: String): U2 = stringMap.getOrElse(
    s, {
  ...
  def addStringConstant(strID: U2): U2 = stringConstMap.getOrElse(
    strID, {
  ...
  def addClass(nameID: U2): U2 = classMap.getOrElse(
    nameID, {
  ...
  def addFieldRef(classID: U2, natID: U2): U2 = fieldRefMap.getOrElse(
    (classID, natID), {
  ...
  def addMethodRef(classID: U2, natID: U2): U2 = methodRefMap.getOrElse(
    (classID, natID), {
  ...
  def addInterfaceMethodRef(classID: U2, natID: U2): U2 =
    methodRefMap.getOrElse(
      (classID, natID), {
  ...
  def addNameAndType(nameID: U2, typeID: U2): U2 = nameAndTypeMap.getOrElse(
    (nameID, typeID), {
  ...
  /** The following methods encode numerical values into their byte
    * representation.
    */
  private def encodeInt(i: Int): U4 = i
  private def encodeFloat(f: Float): U4 = java.lang.Float.floatToIntBits(f)
  private def encodeLong(l: Long): (U4, U4) =
    ((l >>> 32).asInstanceOf[U4], (l & 0xffffffff).asInstanceOf[U4])
  private def encodeDouble(d: Double): (U4, U4) = encodeLong(
    java.lang.Double.doubleToLongBits(d)
  )

  /** Encodes a string into the unusual UTF8-like encoding used in the class
    * file format.
    */
